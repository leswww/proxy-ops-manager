...
interface VpsMonitoringPanelProps {
  vpsAssetId: string
...
export function VpsMonitoringPanel({ vpsAssetId, monitoringMode }: VpsMonitoringPanelProps) {
  ...
  const fetchMetrics = useCallback(async () => {
    try {
      const res = await fetch(`/api/monitoring/vps/${vpsAssetId}`)
      if (!res.ok) {
  ...
  const handleRefresh = async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/monitoring/vps/${vpsAssetId}`, { method: 'POST' })
      if (!res.ok) {
        const data = await res.json()
  ...
  if (monitoringMode === 'DISABLED' && !isDemo) {
    return (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-gray-50/80 rounded-xl p-4 flex flex-col items-center relative">
          <CircularGauge value={metrics.cpuUsagePercent} label="CPU 使用率" subLabel={metrics.cpuCores ? `${metrics.cpuCores} 核` : undefined} />
        </div>
        <div className="bg-gray-50/80 rounded-xl p-4 flex flex-col items-center relative">
          <CircularGauge value={metrics.memoryUsagePercent} label="内存使用率" subLabel={metrics.memoryTotalMb ? `${Math.round(metrics.memoryUsedMb || 0)} / ${metrics.memoryTotalMb} MB` : undefined} />
        </div>
        <div className="bg-gray-50/80 rounded-xl p-4 flex flex-col items-center relative">
          <CircularGauge value={metrics.swapUsagePercent} label="交换分区" subLabel={metrics.swapTotalMb ? `${Math.round(metrics.swapUsedMb || 0)} / ${metrics.swapTotalMb} MB` : undefined} />
        </div>
        <div className="bg-gray-50/80 rounded-xl p-4 flex flex-col items-center relative">
          <CircularGauge value={metrics.diskUsagePercent} label="硬盘使用率" subLabel={metrics.diskTotalGb ? `${(metrics.diskUsedGb || 0).toFixed(1)} / ${metrics.diskTotalGb} GB` : undefined} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <MetricCard label="系统运行时间" value={formatUptime(metrics.systemUptimeSeconds)} />
        <MetricCard label="负载信息" value={metrics.loadAverage || '-'} />
      </div>
